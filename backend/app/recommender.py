...
REQUIRED_COLUMNS = {"record_id", "goodreads_book_id", "title", "authors", "average_rating", "tags_string"}
...
class ContentBasedBookRecommender:
    ...
    def __init__(self, books_path: str, experiments_path: str | None = None):
    ...
    def load(self) -> None:
        ...
        self.books_df["ratings_count"] = pd.to_numeric(self.books_df["ratings_count"], errors="coerce").fillna(1)
    ...
    def model_info(self) -> dict:
        ...
        if not self.experiments_df.empty and "Precision@10" in self.experiments_df:
            row = self.experiments_df.sort_values(["Precision@10", "F1@10"], ascending=False).iloc[0]
            best = row.to_dict()
        return {
    ...
    def _apply_hybrid_score(self, df: pd.DataFrame, similarity_col: str | None, similarity_weight: float, rating_weight: float) -> pd.DataFrame:
        df = df.copy()
        df["norm_rating"] = self._normalize_ratings(df)
        if similarity_col and similarity_col in df.columns:
            df["hybrid_score"] = similarity_weight * df[similarity_col] + rating_weight * df["norm_rating"]
        else:
    ...
    def _format(self, df: pd.DataFrame, n: int) -> list[dict]:
        cols = [c for c in ["record_id", "title", "authors", "average_rating", "ratings_count", "popularity_score", "similarity", "hybrid_score", "tags_string"] if c in df.columns]
    ...
    def global_popular(self, n: int = 10, hybrid: bool = True, similarity_weight: float = SIMILARITY_WEIGHT_DEFAULT, rating_weight: float = RATING_WEIGHT_DEFAULT) -> list[dict]:
        assert self.books_df is not None
    ...
    def genre_popular(self, genre: str, n: int = 10, hybrid: bool = True, similarity_weight: float = SIMILARITY_WEIGHT_DEFAULT, rating_weight: float = RATING_WEIGHT_DEFAULT) -> list[dict]:
        assert self.books_df is not None
    ...
    def by_title(self, title: str, n: int = 10, hybrid: bool = True, similarity_weight: float = SIMILARITY_WEIGHT_DEFAULT, rating_weight: float = RATING_WEIGHT_DEFAULT) -> list[dict]:
        assert self.books_df is not None and self.indices is not None and self.cosine_sim is not None
        if title not in self.indices:
            return []
        idx = self.indices[title]
        if isinstance(idx, pd.Series):
            idx = idx.iloc[0]
        sim_scores = sorted(enumerate(self.cosine_sim[idx]), key=lambda x: x[1], reverse=True)[1:n + 25]
        book_indices = [i for i, _ in sim_scores]
        df = self.books_df.iloc[book_indices].copy()
        df["similarity"] = [s for _, s in sim_scores]
        if hybrid:
            df = self._apply_hybrid_score(df, "similarity", similarity_weight, rating_weight)
        return self._format(df, n)

    def user_profile(self, favorite_titles: Iterable[str], n: int = 10, hybrid: bool = True, similarity_weight: float = SIMILARITY_WEIGHT_DEFAULT, rating_weight: float = RATING_WEIGHT_DEFAULT) -> list[dict]:
        assert self.books_df is not None and self.indices is not None and self.tfidf_matrix is not None
        valid_indices = []
        for title in favorite_titles:
            if title in self.indices:
                idx = self.indices[title]
                valid_indices.append(int(idx.iloc[0] if isinstance(idx, pd.Series) else idx))
    ...
    def final(self, user_input, n: int = 10, hybrid: bool = True, similarity_weight: float = SIMILARITY_WEIGHT_DEFAULT, rating_weight: float = RATING_WEIGHT_DEFAULT) -> dict:
        if user_input is None or user_input == "":
            return {"strategy": "Cold Start Level 1: Global popularity", "items": self.global_popular(n, hybrid, similarity_weight, rating_weight)}
        if isinstance(user_input, list):
            return {"strategy": "User Profile Recommender: average TF-IDF vector of favorite books", "items": self.user_profile(user_input, n, hybrid, similarity_weight, rating_weight)}
        words = str(user_input).strip().split()
        if len(words) <= 3:
            return {"strategy": f"Cold Start Level 2: Genre popularity for '{user_input}'", "items": self.genre_popular(user_input, n, hybrid, similarity_weight, rating_weight)}
        title_recs = self.by_title(str(user_input), n, hybrid, similarity_weight, rating_weight)
        if title_recs:
            return {"strategy": f"Content-based filtering for '{user_input}'", "items": title_recs}
        return {"strategy": f"Fallback: genre popularity for '{user_input}'", "items": self.genre_popular(str(user_input), n, hybrid, similarity_weight, rating_weight)}

    def save_feedback(self, mode: str, user_input: str, title: str, decision: str, comment: str = "") -> dict:
        ...
        row.to_csv(path, mode="a", header=not os.path.exists(path) or os.path.getsize(path) == 0, index=False)
