...
BOOKS_MODEL_PATH = os.environ.get("BOOKS_MODEL_PATH", "/app/data/sample/books_model.csv")
EXPERIMENTS_PATH = os.environ.get("EXPERIMENTS_PATH", "/app/results/experiment_results.csv")
...
def recommend(req: RecommendRequest) -> dict[str, Any]:
    ...
    if req.mode == "global":
        return {"strategy": "Cold Start Level 1: Global popularity", "items": model.global_popular(req.n, req.hybrid, sw, rw)}
    if req.mode == "genre":
        if not req.genre:
            raise HTTPException(400, "genre is required")
        return {"strategy": f"Cold Start Level 2: Genre popularity for '{req.genre}'", "items": model.genre_popular(req.genre, req.n, req.hybrid, sw, rw)}
    if req.mode == "profile":
        if not req.favorite_titles:
            raise HTTPException(400, "favorite_titles is required")
        return {"strategy": "User Profile Recommender", "items": model.user_profile(req.favorite_titles, req.n, req.hybrid, sw, rw)}
    if req.mode == "auto":
        user_input = req.favorite_titles if req.favorite_titles else (req.title or req.genre or None)
        return model.final(user_input, req.n, req.hybrid, sw, rw)
    if not req.title:
        raise HTTPException(400, "title is required")
    return {"strategy": f"Content-based filtering for '{req.title}'", "items": model.by_title(req.title, req.n, req.hybrid, sw, rw)}
...
def feedback(req: FeedbackRequest) -> dict[str, Any]:
    ...
    return model.save_feedback(req.mode, req.user_input, req.title, req.decision, req.comment)
...
async def upload_books_model(file: UploadFile = File(...)) -> dict[str, Any]:
    ...
    # Validate by loading into a temporary model, then replace global model.
    global model
    model = ContentBasedBookRecommender(ACTIVE_UPLOAD_PATH, EXPERIMENTS_PATH)
    return {"uploaded": True, "active_path": ACTIVE_UPLOAD_PATH, "model_info": model.model_info}
